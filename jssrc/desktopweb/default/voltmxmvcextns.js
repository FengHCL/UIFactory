...
(function() {
    ...
    BusinessController_CommandExecutionEngine = function(Command, CommandHandler, ClassExtensionUtility) {
        ...
        CommandExecutionEngine.prototype.registerCommandHandler = function(commandHandler) {
            ...
            this.commandHandlers.push(commandHandlerArray); //voltmx.print("MDA2*** commandHandlers is :" + this.commandHandlers);
        };
        ...
        CommandExecutionEngine.prototype._executeCommand = function(command, params) {
            voltmx.print('MDA2*** CommandExecutionEngine execute : ' + command);
                ...
                for (var i = 0; i < this.commandHandlers.length; i++) {
                    ...
                    if (commandHandlerArray[0] === command.id) {
                        ...
                        if (commandHandlerExtension != null && commandHandlerExtension.execute) {
                            ...
                            if (returnVal === false) {
                                commandHandler.sendResponse(command, voltmx.mvc.constants.STATUS_ABORT, null);
                            }
                        } else {
                            commandHandler.execute(command);
                        }
    ...
    BusinessController_BusinessController = function(CommandExecutionEngine, CommandHandler, Command) {
        ...
        BusinessController.prototype.execute = function(command) {
            //voltmx.print ("In BC executing :" + command.id);
    ...
    BusinessController_BusinessDelegator = function() {
        ...
        BusinessDelegator.prototype.super = function(methodName, argList) {
            ...
            if (this.superParams.refStack.length !== 0) {
                ...
                voltmx.print('#MDA2 : Can\'t find any super for the ' + methodName + ' Method.');
            }
            if (this.superParams.counter === 0) {
    ...
    DataModel_QueryBuilder = function() {
        ...
        QueryBuilder.prototype.buildSelectList = function(baseModel, config) {
            ...
            for (var key in baseModel.relations) {
                ...
                var modeldefination = voltmx.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition(modelstr);
                var configdefination = voltmx.mvc.MDAApplication.getSharedInstance().modelStore.getConfig(modelstr);
    ...
    DataModel_constants = {
        ...
        FORM_CONTROLLER_TYPE: 'voltmx.mvc.MDAFormController',
    ...
    DataModel_DBAssembler = function(Error) {
        var assembler = {};
        /**
         * @function {function defineObjectAssign}
         * This API defines the Object.assign property of JS if it is not defined. Object.assign is not defined in IE.
         * The logic is copy of PollyFill defined for Object.assign in Mozilla blog. PollyFill
         * are compatible with ES5 standard.
         * Below link contains the source of the code
         * https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/assign#Polyfill
         */
        assembler.defineObjectAssign = function() {
            if (!Object.assign) {
        ...
        assembler.fromDBJson = function(BaseModelClass, config, dbJson) {
            ...
            for (var attribute in attributeMap) {
                ...
                if (model) {
                    var modelDefinition = voltmx.mvc.MDAApplication.getSharedInstance().modelStore.getModelDefinition(model);
                    var modelConfig = voltmx.mvc.MDAApplication.getSharedInstance().modelStore.getConfig(model);
    ...
    DataModel_DataSource = function(QueryBuilder, constants, DBAssembler) {
        ...
        DataSource.prototype.getByPrimaryKey = function(BaseModel, config, primaryKey, onCompletion, accessMode, headerParams, dtoOptions) {
            ...
            for (var key in config.primaryKeys) {
                if (!(config.primaryKeys[key] in primaryKey)) {
                    ...
                    onCompletion(voltmx.mvc.constants.STATUS_FAILURE, undefined, errorobject);
                    return;
                }
            }
            ...
            if (accessMode === undefined) {
                accessMode = voltmx.mvc.MDAApplication.getSharedInstance().getAppMode();
            }
            objSvc = voltmx.sdk.getCurrentInstance().getObjectService(config.serviceName, accessMode);
            var headers = {};
            headers[constants.SESSIONTOKEN] = voltmx.mvc.MDAApplication.getSharedInstance().appContext.session_token;
            ...
            var dataObject = new voltmx.sdk.dto.DataObject(config.tableName);
            DataSource.prototype.addOptionsToDTO(dataObject, dtoOptions);
            var odataQueryFlag = voltmx.mvc.MDAApplication.getSharedInstance().getODataStatus();
            if (odataQueryFlag) {
            ...
            var httpOptions = voltmx.mvc.MDAApplication.getSharedInstance().appContext.httpRequestOptions;
            var xmlOptions = voltmx.mvc.MDAApplication.getSharedInstance().appContext.xmlRequestOptions;
            ...
            objSvc.fetch(options, function(response) {
                ...
                onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, modelobject, null);
            }, function(err) {
                //callback sends status, data, error
        ...
        DataSource.prototype.checkObj = function(criteria, finalCriteria) {
            if (criteria.hasOwnProperty(constants.LHS_STRING) && criteria[constants.LHS_STRING].constructor == voltmx.mvc.Expression) {
            ...
            if (criteria.hasOwnProperty(constants.RHS_STRING) && criteria[constants.RHS_STRING].constructor == voltmx.mvc.Expression) {
        ...
        DataSource.prototype.customVerb = function(BaseModel, config, customVerb, params, onCompletion, headerParams, dtoOptions) {
            ...
            var objSvc = voltmx.sdk.getCurrentInstance().getObjectService(config.serviceName, access);
            var headers = {};
            headers[constants.SESSIONTOKEN] = voltmx.mvc.MDAApplication.getSharedInstance().appContext.session_token;
            for (var headKey in headerParams) {
                headers[headKey] = headerParams[headKey];
            ...
            var dataObject = new voltmx.sdk.dto.DataObject(config.tableName);
            DataSource.prototype.addOptionsToDTO(dataObject, dtoOptions);
            if (params != null && params != undefined && params instanceof voltmx.mvc.Data.BaseModel) {
                var dbo = DBAssembler.toDBJson(params, config);
                for (var key in params) {
                    if (params[key] != null && params[key] != undefined) {
                        var descriptor = Object.getOwnPropertyDescriptor(params, key);
                        if (descriptor != null && descriptor != undefined && descriptor['get']) params[key] = dbo[key];
                    }
            ...
            objSvc.customVerb(customVerb, options, function(response) {
                if (response[config.tableName]) {
                    ...
                    onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, models, null);
                } else {
                    for (var attribute in BaseModel.prototype.attributeMap) {
                        delete BaseModel.prototype.attributeMap[attribute];
                    }
                    if (response.text === null) {
                        ...
                        if (errorObject.code == 300) {
                            ...
                            onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, errorObject, null);
                        }
                    }
                    onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, response, null);
                }
            }, function(err) {
                //callback sends status, data, error
                errorObject = DBAssembler.createError(BaseModel, config, err);
        ...
        DataSource.prototype.getByCriteria = function(modelDefination, config, criteria, onCompletion, accessMode, headerParams, dtoOptions) {
            ...
            if (accessMode[constants.ACCESS] == constants.OFFLINE) {
                ...
                objSvc = voltmx.sdk.getCurrentInstance().getObjectService(config.serviceName, access);
                ...
                if (criteria !== undefined) {
                    if (criteria.hasOwnProperty(constants.EXPR)) {
                        ...
                        if (!(criteria[constants.LHS_STRING].constructor == voltmx.mvc.Expression && criteria[constants.RHS_STRING].constructor == voltmx.mvc.Expression)) {
                            finalCriteria[criteria[constants.LHS_STRING]] = criteria[constants.RHS_STRING];
                        }
                    }
                }
                ...
                if (odataQueryFlag) {
                    var queryBuilder = new QueryBuilder();
                    var odataQuery = queryBuilder.constructPrimaryKeyODataQuery(finalCriteria);
                    if (odataQuery) dataObject.setOdataUrl(odataQuery);
                } else {
        ...
        DataSource.prototype.save = function(BaseModel, config, onCompletion, accessMode, headerParams, dtoOptions) {
            ...
            objSvc.create(options, function(res) {
                voltmx.print(constants.MESSAGE_FOR_RECORDCREATED + JSON.stringify(res));
                voltmx.print(constants.MESSAGE_FOR_RECORDCREATED);
                //callback sends status, data, error
                if (res.text === null) {
                    errorObject = DBAssembler.createError(BaseModel, config, response);
                    onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, errorObject, null);
                }
                onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, res, null);
            }, function(err) {
                voltmx.print(JSON.stringify(err));
                errorObject = DBAssembler.createError(BaseModel, config, err);
                onCompletion(voltmx.mvc.constants.STATUS_FAILURE, undefined, errorObject); //callback sends status, data, error
                //onCompletion(voltmx.mvc.constants.STATUS_FAILURE,undefined,err);
            });
        };
        /**
        ...
        DataSource.prototype.partialUpdate = function(BaseModel, config, onCompletion, accessMode, headerParams, dtoOptions) {
            ...
            objSvc.partialUpdate(options, function(res) {
                voltmx.print(constants.MESSAGE_FOR_RECORDPARIALUPDATE + JSON.stringify(res));
                voltmx.print(constants.MESSAGE_FOR_RECORDPARIALUPDATE);
                //callback sends status, data, error
                if (res.text === null) {
                    errorObject = DBAssembler.createError(BaseModel, config, response);
                    onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, errorObject, null);
        ...
        DataSource.prototype.update = function(BaseModel, config, onCompletion, accessMode, headerParams, dtoOptions) {
            ...
            objSvc.update(options, function(res) {
                voltmx.print(constants.MESSAGE_FOR_RECORDUPDATED + JSON.stringify(res));
                voltmx.print(constants.MESSAGE_FOR_RECORDUPDATED);
                //callback sends status, data, error
                if (res.text === null) {
                    errorObject = DBAssembler.createError(BaseModel, config, response);
                    onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, errorObject, null);
                ...
                //callback sends status, data, error
                errorObject = DBAssembler.createError(BaseModel, config, err);
                onCompletion(voltmx.mvc.constants.STATUS_FAILURE, undefined, errorObject); //onCompletion(voltmx.mvc.constants.STATUS_FAILURE,undefined,err);
            });
        };
        /**
        ...
        DataSource.prototype.remove = function(BaseModel, config, onCompletion, accessMode, headerParams, dtoOptions) {
            ...
            objSvc.deleteRecord(options, function(res) {
                voltmx.print(constants.MESSAGE_FOR_RECORDDELETED);
                onCompletion(voltmx.mvc.constants.STATUS_SUCCESS, res, null);
            }, function(err) {
                voltmx.print(constants.ERROR_IN_RECORD_DELETION + JSON.stringify(err));
                errorObject = DBAssembler.createError(BaseModel, config, err);
                onCompletion(voltmx.mvc.constants.STATUS_FAILURE, undefined, errorObject); //onCompletion(voltmx.mvc.constants.STATUS_FAILURE, undefined, err);
            });
        };
    ...
    PresentationController_MDABasePresenter = function() {
        ...
        MDABasePresenter.prototype.asyncUpdateUI = function(uiTag, context) {
            ...
            if (formInStack) {
                var controller = _voltmx.mvc.GetController(uiTag, true);
                controller.updateUI(context);
            } else {
                voltmx.print('## MDA: Trying to asyncUpdateUI. Can\'t find the FORM: ' + uiTag + ' as Loaded.');
            }
        };
        ...
        MDABasePresenter.prototype.clearPresentationStackUntil = function(formName) {
            if (formName) {
                ...
                voltmx.print('mention form name parameter to clear until that object in stack');
            }
        };
        MDABasePresenter.prototype.presentPreviousScreen = function(context) {
            ...
            if (previousStackContext) {
                ...
                voltmx.print('No previous forms available');
            }
        };
    ...
    BaseNavigator_MDABaseNavigator = function() {
        ...
        MDABaseNavigator.prototype.presentForm = function(form, uiTag, viewModel) {
            ...
            var _currentForm = voltmx.application.getCurrentForm();
            if (this._asyncNavStack.hasOwnProperty(form)) {
                ...
                if (_currentForm && form == _currentForm.id) {
                    var controller = _voltmx.mvc.GetController(form, true);
                    controller.updateUI(context['viewModel']);
                } else {
                    try {
                        this._asyncNavStack[form] = [];
                        var frmNavObject = new voltmx.mvc.Navigation(form);
    ...
    ModuleManager_MDAModule = function(MDABasePresenter, MDABaseNavigator, BusinessController, constants, BusinessDelegator) {
        function MDAModule(moduleName, moduleMode, channel, callback, isStartup) {
            ...
            try {
                ...
                if (this.moduleConfig[moduleMode]) {
                    ...
                    voltmx.print('MDA2***..module: ' + moduleName + '- Default Mode');
                }
            } catch (e) {
        ...
        MDAModule.prototype.setStartupModule = function(callback) {
            voltmx.print('MDA2*** Setup Startup module');
            if (callback !== undefined) {
                this.setUpBusinessPresentationController(callback);
            } else {
                if (this.moduleConfig.ModuleType === undefined || this.moduleConfig.ModuleType == 'Manager') {
                    this.setUpBusinessController();
                }
                if (this.moduleConfig.ModuleType === undefined || this.moduleConfig.ModuleType == 'UIModule') {
        ...
        MDAModule.prototype.setupModule = function(callback) {
            voltmx.print('MDA2*** Setup module');
            // setUpForms() maps form friendlyName and its path in registry,required for navigate(<form friendlyname>) to work.
            this.setUpForms();
            if (callback !== undefined) {
                this.setUpBusinessPresentationController(callback);
            } else {
        ...
        MDAModule.prototype.setUpBusinessPresentationController = function(callback) {
            if (this.moduleConfig === undefined) {
                ...
                if (this.businessController === null && this.presentationController === null) {
                    voltmx.print('MDA2*** Derived module does not provide a Business controller');
                ...
                var currentChannel = voltmx.sdk.getChannelType();
                ...
                if (this.moduleConfig.ModuleType === undefined && (businessControllerClass === undefined || businessControllerClass === '' || presentationControllerClass === undefined || presentationControllerClass === '')) {
                    voltmx.print(' MDA2*** Config does not provide a Business controller');
                    voltmx.print('MDA2*** Config does not provide a Presentation controller');
                    this.businessController = this.createBusinessController();
                    this.presentationController = this.createPresentationController();
                    if (this.businessController === null && this.presentationController === null) {
                        voltmx.print('MDA2*** Derived module does not provide a Business controller');
                        this.businessController = this.createDefaultBusinessController();
                        voltmx.print('MDA2*** Derived module does not provide a Presentation controller');
                    ...
                    try {
                        ...
                        var DerivedController = require(params, function() {
                            ...
                            if (presentationControllerClass) pController = new presentationControllerClass();
                            if (businessControllerClass) bController = new businessControllerClass();
                            self.businessController = bController;
                            self.presentationController = pController;
                            if (self.moduleConfig.ModuleType === undefined || self.moduleConfig.ModuleType == 'UIModule') {
                                self.setUpNavigator();
                            ...
                            var configureBusinessController = function() {
                            ...
                            if (self.moduleConfig.ModuleType != undefined && self.moduleConfig.ModuleType == 'Manager') configureBusinessController();
                            if (self.moduleConfig.ModuleType != undefined && self.moduleConfig.ModuleType == 'UIModule') self.createPresentationExtensionsAsync(Callback);
                            if (self.moduleConfig.ModuleType === undefined) self.createPresentationExtensionsAsync(configureBusinessController);
                        });
                    } catch (err) {
                        throw new Exception('ERROR_CODE_300', ' Invalid Controller Class ' + err);
                    }
                }
            }
        };
            ...
            if (businessExtensions !== undefined) {
                ...
                try {
                    (function() {
                        ...
                        voltmx.print('MDA2**** extNModule : ' + extNModule);
                        _voltmx.mvc.assignFunctions2Controller(self.businessController, extNModule);
                        callback(self);
                    }));
                } catch (err) {
                ...
                callback(self);
                voltmx.print('MDA2**** invalid businessExtensions: ' + businessExtensions);
            }
        };
                ...
                if (presentationExtensions !== undefined) {
                    ...
                    try {
                        (function() {
                            ...
                            voltmx.print('MDA2**** extNModule : ' + extNModule);
                            _voltmx.mvc.assignFunctions2Controller(self.presentationController, extNModule);
                            callback();
                        }));
                    } catch (err) {
        ...
        MDAModule.prototype.setUpBusinessController = function() {
            if (this.moduleConfig === undefined) {
                ...
                if (businessControllerClass === undefined || businessControllerClass === '') {
                    ...
                    voltmx.print('MDA2*** Config  provides a Business controller');
                    try {
        ...
        MDAModule.prototype.setUpPresentationController = function() {
            if (this.moduleConfig === undefined) {
                ...
                if (presentationControllerClass === undefined || presentationControllerClass === '') {
                    ...
                    voltmx.print('MDA2*** Config  provides a Presentation controller');
                    try {
        ...
        MDAModule.prototype.setUpForms = function() {
            if (this.moduleConfig !== undefined) {
                ...
                var channel = voltmx.sdk.getChannelType();
                //Call some API here
                if (forms && channel && forms[channel]) {
                    var formsList = Object.keys(forms[channel]);
                    for (var eachForm in formsList) {
                        if (voltmx.mvc.registry.getControllerType(eachForm) != constants.FORM_CONTROLLER_TYPE) {
                            ...
                            voltmx.mvc.registry.add(friendlyName, formPath, {
        ...
        MDAModule.prototype.createPresentationExtensions = function() {
            if (this.moduleConfig !== undefined) {
                ...
                if (presentationExtensions !== undefined) {
                    ...
                    _voltmx.mvc.assignFunctions2Controller(this.presentationController, extNModule);
                } else {
                    voltmx.print('MDA2**** invalid presentationExtensions: ' + presentationExtensions);
                }
            }
        };
        MDAModule.prototype.createBusinessExtensions = function() {
            if (this.moduleConfig !== undefined) {
                ...
                if (businessExtensions !== undefined) {
                    ...
                    _voltmx.mvc.assignFunctions2Controller(this.businessController, extNModule);
                } else {
                    voltmx.print('MDA2**** invalid businessExtensions: ' + businessExtensions);
                }
            }
    ...
    ModuleManager_MDAModuleManager = function(MDAModule) {
        ...
        MDAModuleManager.prototype.loadModule = function(moduleName, moduleMode, channel, callback) {
            ...
            voltmx.print('MDA2***..loadModule: ' + moduleName + '-' + moduleMode + 'Mode');
            var self = this;
        ...
        MDAModuleManager.prototype.getModule = function(moduleName, moduleMode, channel, callback) {
            // voltmx.print('Returned MDA2*** Module: ' + moduleName + '-' + this.moduleMap[moduleName].moduleMode + ' Mode Instance'); 
            if (!this.moduleMap[moduleName]) {
        ...
        MDAModuleManager.prototype.loadStartupModule = function(moduleName, moduleMode, channel, callback) {
            if (moduleMode == null) moduleMode = 'Default';
            voltmx.print('MDA2***..loadStartupModule: ' + moduleName + '-' + moduleMode + 'Mode');
            var self = this;
            var isStartup = 1;
            if (!this.moduleMap[moduleName]) {
                var module = null;
                ...
                if (widgetDataModel == dataModelKeys[eachDataModel]) {
                    ...
                    if (widgetType in this.map) {
                        ...
                        try {
                            ...
                            voltmx.print('Widget Data Mapper error while mapping: ' + err);
                        }
                    } else {
                        voltmx.print('Widget doesn\'t exist with UIBinder ' + widgetType);
                    }
                } else {
                    voltmx.print('View Model ' + dataModelKeys[eachDataModel] + ' is not for widget:' + widgetId + ' with View Model:' + widgetDataModel);
                }
            }
        };
    ...
    UIBinder_WidgetDataMapper_WidgetDataMapper = function() {
        ...
        WidgetDataMapper.prototype.mapData = function(widgetConfig, data, widget) {
            if (widgetConfig.propertyMapper) {
                voltmx.print('Mapping properties with widgets');
                    ...
                    try {
                        if (widgetConfig.scope.toLowerCase() == 'context') {} else {
                            ...
                            try {
                                ...
                                voltmx.print('MapData function error while mapping property ' + err);
                            }
                        }
                    } catch (err) {
                        voltmx.print('Error in data format : MapData WidgetDataMapper ' + err);
                    }
                }
            } else {
                //If no property map exists
                //Backward compatibility
                voltmx.print('Mapping default properties with widgets');
                ...
                if (typeof data[widgetConfig.entityId][widgetConfig.fieldId] == 'object') {
                    ...
                    try {
                        ...
                        voltmx.print('Error while Mapping default property with widget ' + err);
                    }
                } else {
                    try {
        ...
        Logger.logExecutionTimes = function(targetObject, targetMethod, args) {
            function callbackTimeStamping(callback) {
                return function() {
                    ...
                    voltmx.print('MDA PERF :: Start Time executing callback ' + callback.name + '() : ' + startTime);
                    callback.apply(null, arguments);
                    var endTime = Date.now();
                    voltmx.print('MDA PERF :: End Time executing callback ' + callback.name + '() : ' + endTime);
                    voltmx.print('MDA PERF :: Total time executing callback ' + callback.name + '() : ' + (endTime - startTime) + 'ms');
                };
            }
            var argArr = [];
            for (var i = 0; i < args.length; i++) {
                ...
                } else if (args[i] instanceof voltmx.mvc.Business.Command) {
                    var target = {};
            ...
            var startTime = Date.now();
            voltmx.print(startTimeLogPrefix + startTime);
            var returnVal = targetFunction.apply(_targetObject, argArr);
            var endTime = Date.now();
            voltmx.print(endTimeLogPrefix + endTime);
            voltmx.print(totalTimeLogPrefix + (endTime - startTime) + 'ms');
            return returnVal;
        };
        return Logger;
    }();
    ...
    commonUtils_ProcessorUtils = function() {
        ...
        ProcessorUtils.applyFunction = function(fnToApply, val, context) {
            ...
            if (fnToApply && context['metadata']) {
                var config = voltmx.mvc.MDAApplication.getSharedInstance().modelStore.getConfig(context['object']);
        ...
        ProcessorUtils.getMetadataForObject = function(serviceName, objectName, options, successCallback, failureCallback) {
            voltmx.sdk.getCurrentInstance().getObjectService(serviceName, {
                'access': 'online'
    ...
    DataModel_BaseModel = function(ModelRelation) {
        ...
        function dummyCompletionCallback(status, response, error) {
        ...
        BaseModel.prototype.save = function(onCompletion, accessMode, options) {
            ...
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).save(this, onCompletion, accessMode, options);
        };
        /**
         * @function getByPrimaryKey  fetches data for the specified model based on the primary keys
         * @param  {String/JSON} primaryKey   primitive in case of one primary key and json in case of composite primary key
        ...
        BaseModel.getByPrimaryKey = function(PrimaryKey, onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).getByPrimaryKey(PrimaryKey, onCompletion, accessMode, options);
        };
        /**
         * @function getByPrimaryKey  fetches data for the specified model based on the primary keys
         * @param  {String/JSON} primaryKey   primitive in case of one primary key and json in case of composite primary key
         ...
         * @return
         */
        BaseModel.prototype.getByPrimaryKey = function(PrimaryKey, onCompletion, accessMode, options) {
            var model = this.objModelName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).getByPrimaryKey(PrimaryKey, onCompletion, accessMode, options);
        ...
        BaseModel.prototype.update = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).update(this, onCompletion, accessMode, options);
        };
        /**
         * @function partialUpdate partially update the model data to the backend
         * @param  {function} onCompletion success callback
        ...
        BaseModel.prototype.partialUpdate = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).partialUpdate(this, onCompletion, accessMode, options);
        };
        /**
         * @function remove deletes the specified model from the backend
         * @param  {function} onCompletion success callback
        ...
        BaseModel.prototype.remove = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).remove(this, onCompletion, accessMode, options);
        };
        /**
        ...
        BaseModel.removeById = function(primaryKey, onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).removeById(this, primaryKey, onCompletion, accessMode, options);
        };
        /**
         * @function removeById deletes the specified model from the backend with a specific id
         * @param {JSON/STRING} primaryKey primary key value or json representing the key and value
         ...
         * @return
         */
        BaseModel.prototype.removeById = function(primaryKey, onCompletion, accessMode, options) {
            var model = this.objModelName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).removeById(this, primaryKey, onCompletion, accessMode, options);
        ...
        BaseModel.getAll = function(onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).getAll(onCompletion, accessMode, options);
        };
        /**
         * @function getAll  fetches  all the data for the specified model
         * @param  {type} onCompletion success callback
         ...
         * @return
         */
        BaseModel.prototype.getAll = function(onCompletion, accessMode, options) {
            var model = this.objModelName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).getAll(onCompletion, accessMode, options);
        ...
        BaseModel.customVerb = function(customVerb, params, onCompletion, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).customVerb(customVerb, params, onCompletion, options);
        };
        /**
         * @function customVerb fetches data for the specified model using specified customVerb
         * @param  {String} customVerb   name of the custom verb to be executed
         ...
         * @return
         */
        BaseModel.prototype.customVerb = function(customVerb, params, onCompletion, options) {
            var model = this.objModelName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).customVerb(customVerb, params, onCompletion, options);
        ...
        BaseModel.getByCriteria = function(criteria, onCompletion, accessMode, options) {
            ...
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).getByCriteria(criteria, onCompletion, accessMode, options);
        };
        /**
         * @function getByCriteria  fetches data for the specified model based on the criteria passed
         * @param  {Json} criteria        criteria object based on which query is build in case of online and offline
         ...
         * @return
         */
        BaseModel.prototype.getByCriteria = function(criteria, onCompletion, accessMode, options) {
            var model = this.objModelName;
            options = options ? options : null;
            onCompletion = onCompletion ? onCompletion : dummyCompletionCallback;
            voltmx.mvc.MDAApplication.getSharedInstance().getRepoManager().getRepository(model).getByCriteria(criteria, onCompletion, accessMode, options);
    ...
    FormController_MDAFormController = function(inheritsFrom) {
        function MDAFormController(viewId1) {
            ...
            voltmx.mvc.FormController.call(this, viewId1);
        }
        inheritsFrom(MDAFormController, voltmx.mvc.FormController);
        MDAFormController.prototype.show = function(context, isBackNavigation) {
            voltmx.mvc.FormController.prototype.show.call(this, context, isBackNavigation);
        };
        ...
        MDAFormController.prototype.onNavigate = function(context, isBackNavigation) {
            ...
            if (context) {
                ...
                // Refresh the form once per every context if available.
                // Updating the form per context could have perf implications
        ...
        MDAFormController.prototype.updateUI = function(viewModel) {
            var uiInstance = voltmx.mvc.MDAApplication.getSharedInstance().getUIBinder();
    ...
    commonUtils_ControllerGetterAPI = function() {
        function getController(formId) {
            var tempController = voltmx.mvc.registry.get(formId);
            var controllerType = 'voltmx.mvc.MDAFormController';
            var isForm = true;
            if (tempController != null) {
                var tempControllerType = voltmx.mvc.registry.getControllerType(formId);
                if (tempControllerType == controllerType) {
                    return _voltmx.mvc.GetController(formId, isForm);
                } else {
                    var friendlyName = voltmx.mvc.registry.getFriendlyName(formId);
                    var controllerName = voltmx.mvc.registry.getControllerName(formId);
                    var controllerExtName = voltmx.mvc.registry.getControllerExtName(formId);
                    voltmx.mvc.registry.remove(formId);
                    voltmx.mvc.registry.add(formId, formId, {
    ...
    main = function(require) {
        ...
        voltmx.mvc = voltmx.mvc ? voltmx.mvc : {};
        const mda = voltmx.mvc;
        mda.Business = {};
        ...
        define('voltmx.mvc.MDAFormController', voltmx.mvc.MDAFormController);
        define('kony/mvc/MDAFormController', voltmx.mvc.MDAFormController);
        define('MDAFormController', voltmx.mvc.MDAFormController);
        return mda;
    }({});
}());